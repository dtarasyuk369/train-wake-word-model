...
class DataTransform:
    """Callable class for data transformation that can be pickled"""
    def __init__(self, n):
        self.n = n

    def __call__(self, x):
        if self.n > x.shape[1] or self.n < x.shape[1]:
            x = np.vstack(x)
            new_batch = np.array([x[i:i+self.n, :] for i in range(0, x.shape[0]-self.n, self.n)])
        else:
            return x
        return new_batch


def resample_clips_to_16k(directory: str):
    ...
    for wav_path in wav_files:
        ...
        if sr != TARGET_SR:
            ...
            resampled += 1
    if resampled > 0:
        logging.info(f"Ресемплировано {resampled}/{len(wav_files)} клипов в {TARGET_SR} Hz")
...
class Model(nn.Module):
    def __init__(self, n_classes=1, input_shape=(16, 96), model_type="dnn",
        ...
        # Define metrics
        if n_classes == 1:
            def binary_fp(pred, y):
                return (y-pred <= -0.5).sum()
    ...
    def export_model(self, model, model_name, output_dir):
        if self.n_classes != 1:
            raise ValueError(
                "Exporting models with more than one class is currently not supported! "
...
if __name__ == '__main__':
    ...
    if args.generate_clips:
        ...
        if n_current_samples <= 0.95*config["n_samples"]:
            ...
            resample_clips_to_16k(positive_train_output_dir)
        ...
        if n_current_samples <= 0.95*config["n_samples_val"]:
            ...
            resample_clips_to_16k(positive_test_output_dir)
            torch.cuda.empty_cache()
        else:
        ...
        if n_current_samples <= 0.95*config["n_samples"]:
            ...
            resample_clips_to_16k(negative_train_output_dir)
            torch.cuda.empty_cache()
        else:
        ...
        if n_current_samples <= 0.95*config["n_samples_val"]:
            ...
            resample_clips_to_16k(negative_test_output_dir)
            torch.cuda.empty_cache()
        else:
    ...
    if args.augment_clips:
        logging.info("Проверка sample rate клипов (ресемплирование в 16 kHz при необходимости)...")
        for d in [positive_train_output_dir, positive_test_output_dir, negative_train_output_dir, negative_test_output_dir]:
            if os.path.isdir(d):
                resample_clips_to_16k(d)

        if not os.path.exists(os.path.join(feature_save_dir, "positive_features_train.npy")) or args.overwrite:
    ...
    if args.train_model:
        ...
        data_transform_func = DataTransform(input_shape[0])

        # Create label transforms as needed for model (currently only supports binary classification models)
        data_transforms = {key: data_transform_func for key in config["feature_data_files"].keys()}
